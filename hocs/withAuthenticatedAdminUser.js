...
export default WrapComponent => {
    const withAuthenticatedAdminUser = (props) => {
        ...
        const checkAuth = async () => {
            if (isAuthen && payload.role === 1 || payload.role === 2 || payload.role === 3) {
                setRender(true)
            }
                ...
                if (jwt !== null) {
                    ...
                    if (result !== 'err' && result.role === 1 || result.role === 2 || result.role === 3) {
                        let payload = await {
                    ...
                    else if (result !== 'err' && result.role !== 1 || result.role !== 2 || result.role !== 3) {
                        removeToken()
                        Router.push('/login')
                    }
                    else if (result === 'err') {
