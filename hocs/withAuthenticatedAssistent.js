...
export default WrapComponent => {
    const withAuthenticatedAssistent = (props) => {
        ...
        const checkAuth = async () => {
            if (isAuthen && payload.role === 2) {
                setRender(true)
            }
            else if (isAuthen && payload.role !== 2) {
                ...
                if (jwt !== null) {
                    ...
                    else if (result !== 'err' && result.role !== 2) {
                        removeToken()
                        Router.push('/login')
                    }
                    else if (result === 'err') {
    ...
    return compose(
        connect(mapStateToProps, { passLogin, setStateDate })
    )(withAuthenticatedAssistent)
}